...
export default function PrestataireDocumentVerification() {
  ...
  // Accepte tous les docs en appelant la route validate pour chaque
  const handleAcceptAll = async (userId: number) => {
    const docs = groupedByUser[userId].docs;
    try {
      await Promise.all(
        docs.map((doc) =>
          fetch(`http://51.15.231.248:3001/admin/documents/${doc.id}/validate`, {
            method: 'POST',
            headers: {
            ...
            if (!r.ok) throw new Error('Échec validation');
          })
        )
      );
      ...
      console.error(err);
      setMessage(err.message || 'Erreur lors de l’acceptation.');
    }
  };

  // Refuse tous les docs via la route refuse-all
  const handleRefuseAll = async (userId: number) => {
    try {
      ...
      console.error(err);
      setMessage(err.message || 'Erreur lors du refus.');
    }
  };
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">
        Vérification des documents par utilisateur
      </h1>

      {message && <p className="mb-4 text-sm text-red-600">{message}</p>}

      <div className="space-y-4">
        {Object.entries(groupedByUser).map(([userIdStr, { userStatus, docs }]) => {
