...
export default function PublicProfileVitrine() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://ecodeli.eu/api/public-profile/${userId}`, {
    ...
    axios
      .get(`https://ecodeli.eu/api/users/${userId}`)
    ...
    axios
      .get(`https://ecodeli.eu/api/rates/provider/${userId}`)
  ...
  const averageRating = rates.length
    ? (
        rates.reduce((sum, r) => sum + r.rating, 0) / rates.length
      ).toFixed(1)
    : null;
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto bg-white rounded shadow">
      ...
      <div className="space-y-3 mb-6">
        ...
        <p>
          <span className="font-semibold">Prix :</span> {profile.price}€
        </p>
        <p>
          ...
          <ul className="space-y-4">
            {rates.map((rate) => (
              <li key={rate.id} className="border p-4 rounded">
                ...
                {rate.comment && (
                  <p className="italic mt-2">“{rate.comment}”</p>
                )}
